...
There are many ways to check for missingness - one great way is to use the `aggr` from the `VIM` package (see Q2). In our case, we can simply check the total amount of missing values
```{r}
sum(is.na(X))
```
...
### Response
We already extracted the response above, but we now assign it to a new variable name and encode it to 0 and 1. We can do this simply by making the output numerical (1,2,3), where 2 identifies control patients.
...
```{r}
saveRDS(list(X,y,group_index),"colitis_data_c3.RDS")
```
